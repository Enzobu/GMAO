...
export async function createMaintenanceType(payload: ConfigurationPayload) {
  const response = await api.post<ConfigurationItem>(
    "/maintenance_types",
    payload,
  )
...
export async function updateMaintenanceType(
  ...
  const response = await api.patch<ConfigurationItem>(
...
export async function createInspectionCenter(
  payload: InspectionCenterConfigurationPayload,
) {
  const response = await api.post<InspectionCenterConfigurationItem>(
    "/inspection_centers",
    payload,
  )
...
export async function updateInspectionCenter(
  id: number,
  payload: InspectionCenterConfigurationPayload,
) {
  const response = await api.patch<InspectionCenterConfigurationItem>(
    `/inspection_centers/${id}`,
    payload,
    { headers: { "Content-Type": "application/merge-patch+json" } },
  )
