...
import type { InspectionCenter, VehicleInspectionEvent, VehicleInspectionPayload, VehicleInsuranceEvent, VehicleInsurancePayload } from "@/types/vehicle-events"
...
export async function getVehicleInsurances() {
  const response = await api.get<ApiCollection<VehicleInsuranceEvent>>("/vehicle_insurances")
  return collectionItems(response.data)
}

export async function getVehicleInsurance(id: string | number) {
  const response = await api.get<VehicleInsuranceEvent>(`/vehicle_insurances/${id}`)
  return response.data
}

export async function createVehicleInsurance(payload: VehicleInsurancePayload) {
  const response = await api.post<VehicleInsuranceEvent>("/vehicle_insurances", payload)
  return response.data
}

export async function updateVehicleInsurance(id: string | number, payload: Partial<VehicleInsurancePayload>) {
  const response = await api.patch<VehicleInsuranceEvent>(`/vehicle_insurances/${id}`, payload, {
    headers: { "Content-Type": "application/merge-patch+json" },
  })
  return response.data
}

export async function closeVehicleInsurance(id: string | number, endDate: string) {
...
export async function getVehicleInspections() {
  const response = await api.get<ApiCollection<VehicleInspectionEvent>>("/vehicle_inspections")
  return collectionItems(response.data)
}

export async function getVehicleInspection(id: string | number) {
  const response = await api.get<VehicleInspectionEvent>(`/vehicle_inspections/${id}`)
  return response.data
}
...
export async function getInspectionCenters() {
  const response = await api.get<ApiCollection<InspectionCenter>>("/inspection_centers")
  return collectionItems(response.data)
}
