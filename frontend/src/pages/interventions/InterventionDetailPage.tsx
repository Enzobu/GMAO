...
export default function InterventionDetailPage({ vehicleScoped = false }: Readonly<{ vehicleScoped?: boolean }>) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      navigate(vehicleScoped && vehicleId ? `/vehicles/${vehicleId}/interventions` : "/interventions")
    } finally {
      setIsDeleting(false)
    }
  }

  if (isLoading) return <div className="text-sm text-muted-foreground">Chargement de l’intervention...</div>
  if (error || !intervention) return <ErrorMessage>{error ?? "Intervention introuvable."}</ErrorMessage>

  const routeVehicleId = vehicleId ?? String(intervention.vehicle.id)
  const backTo = vehicleScoped ? `/vehicles/${routeVehicleId}/interventions` : "/interventions"
  ...
  return (
    <div className="space-y-6">
      <ConfirmDialog
        ...
        description="L’intervention sera masquée et les pièces seront restaurées si elle était réalisée."
      ...
      <InterventionHeader
        ...
        actions={vehicleScoped && (
          <>
            {canEdit && (
              <Button asChild>
                <Link to={`/vehicles/${routeVehicleId}/interventions/${intervention.id}/edit`}><Pencil />Modifier</Link>
              </Button>
            )}
            {isAdmin && (
              <Button variant="destructive" onClick={() => setIsDeleteOpen(true)}><Trash2 />Supprimer</Button>
            )}
          </>
        )}
      />

      <InterventionBadges intervention={intervention} readOnly={!isAdmin && (!vehicleScoped || !canEdit)} />

      <Card>
        <CardHeader><CardTitle>Détails</CardTitle></CardHeader>
        <CardContent className="grid gap-4 text-sm md:grid-cols-2 xl:grid-cols-4">
          <Detail boxed label="Kilométrage" value={`${formatNumber(intervention.mileage)} km`} />
          <Detail boxed label="Prévu" value={formatDateTime(intervention.plannedAt)} />
...
function partKey(part: Intervention["maintenanceParts"] extends (infer T)[] | undefined ? T extends { part: infer P } ? P : never : never) {
  if (typeof part === "string") {
    return part
  }
