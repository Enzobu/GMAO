...
export default function InterventionsPage({ vehicleScoped = false }: Readonly<{ vehicleScoped?: boolean }>) {
  ...
  const [itemsPerPage, setItemsPerPage] = useLocalStorageState<ItemsPerPageValue>("interventions.itemsPerPage", "12")
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const { interventionData, vehicleData } = await loadPageData(vehicleScoped, vehicleId)
  ...
  function openQuickAction(action: QuickAction) {
    ...
    setQuickActionMileage(action.type === "finish" ? String(action.intervention.vehicle.lastMileage ?? action.intervention.mileage ?? "") : "")
  }
  ...
  async function confirmQuickAction(forceMileage = false) {
    ...
    try {
      const patch = quickAction.type === "start"
        ? { status: "in_progress" as const, startedAt: toApiDateTime(quickActionDate) }
        : { status: "completed" as const, finishedAt: toApiDateTime(quickActionDate), mileage: Number(quickActionMileage) }
  ...
  const filtered = useMemo(() => {
    return interventions
      ...
      .filter((item) => {
        const text = `${item.maintenanceType?.name ?? ""} ${item.vehicle?.registration ?? ""} ${item.notes ?? ""}`.toLowerCase()
        return text.includes(query.toLowerCase().trim())
      })
      .sort((a, b) => String(b.finishedAt ?? b.startedAt ?? b.plannedAt ?? b.createdAt ?? "").localeCompare(String(a.finishedAt ?? a.startedAt ?? a.plannedAt ?? a.createdAt ?? "")))
  }, [interventions, query, status])
  const pageSize = itemsPerPage === "all" ? filtered.length || 1 : Number(itemsPerPage)
  const totalPages = itemsPerPage === "all" ? 1 : Math.max(1, Math.ceil(filtered.length / pageSize))
  const currentPage = Math.min(page, totalPages)
  const visible = itemsPerPage === "all" ? filtered : filtered.slice((currentPage - 1) * pageSize, currentPage * pageSize)
  const visibleStart = filtered.length === 0 ? 0 : (currentPage - 1) * pageSize + 1
  const visibleEnd = itemsPerPage === "all" ? filtered.length : Math.min(currentPage * pageSize, filtered.length)

  if (isLoading) return <div className="text-sm text-muted-foreground">Chargement des interventions...</div>
  ...
  return (
    <div className="space-y-6">
      <QuickActionDialog
        ...
        onOpenChange={(open) => !open && !isUpdatingStatus && setQuickAction(null)}
      ...
      <InterventionHeader
        ...
        description={vehicleScoped && vehicle ? vehicleDisplayName(vehicle) : "Liste globale en lecture seule."}
        backTo={vehicleScoped && vehicle ? `/vehicles/${vehicle.id}` : undefined}
        backLabel="Retour au véhicule"
        actions={vehicleScoped && canEditVehicle && vehicle ? (
          <Button asChild>
            <Link to={`/vehicles/${vehicle.id}/interventions/new`}><Plus />Ajouter</Link>
          </Button>
        ) : undefined}
      />

      <div className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-[1fr_14rem]">
        <input
          value={query}
      ...
      <PaginationControls
        ...
        onItemsPerPageChange={(value) => { setItemsPerPage(value as ItemsPerPageValue); setPage(1) }}
  ...
  return (
    <>
      {intervention.status === "todo" && (
        <Button variant="outline" size="sm" onClick={() => onQuickAction({ type: "start", intervention })}>
          <Play />
          Démarrer
        </Button>
      )}
      {intervention.status === "in_progress" && (
        <Button variant="outline" size="sm" onClick={() => onQuickAction({ type: "finish", intervention })}>
          <CheckCircle2 />
          Terminer
        </Button>
      )}
      <Button variant="outline" size="sm" asChild>
        <Link to={`/vehicles/${vehicleId}/interventions/${intervention.id}/edit`}>
          <Pencil />
          Modifier
        </Link>
      </Button>
...
function QuickActionDialog({
  ...
  return (
    <Dialog open={action !== null} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isStart ? "Démarrer l’intervention" : "Terminer l’intervention"}</DialogTitle>
          <DialogDescription>
            {isStart ? "Renseignez la date de début de l’intervention." : "Renseignez la date de fin de l’intervention."}
          </DialogDescription>
        </DialogHeader>
...
function errorMessage(error: unknown) {
  if (isAxiosError(error)) {
    ...
    return data?.detail ?? data?.description ?? data?.message ?? "Impossible de mettre à jour l’intervention."
  }
