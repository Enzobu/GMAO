...
export {
  FormActions as InterventionFormActions,
} from "@/components/form-actions"

export function InterventionBadges({
  intervention,
  readOnly = false,
}: Readonly<{ intervention: Intervention; readOnly?: boolean }>) {
  return (
    <div className="flex flex-wrap gap-2">
      <Badge variant={interventionStatusVariant(intervention.status)}>
        {interventionStatusLabel(intervention.status)}
      </Badge>
      <Badge variant="outline">
        {intervention.maintenanceType?.name ?? "—"}
      </Badge>
      <Badge variant={intervention.isExternal ? "outline" : "secondary"}>
        {intervention.isExternal ? "Externe" : "Interne"}
      </Badge>
      {readOnly && (
        <Badge
          variant="outline"
    ...
    >
      ...
      <CardContent className="space-y-4 p-5">
        ...
        >
          <div className="space-y-2">
            <div className="text-lg font-semibold">
              {intervention.maintenanceType?.name ?? "Intervention"}
            </div>
            <InterventionBadges
              intervention={intervention}
              readOnly={readOnly}
        ...
        <div className="grid gap-3 text-sm md:grid-cols-4">
          <Detail
            label="Véhicule"
            value={intervention.vehicle?.registration?.toUpperCase() ?? "—"}
          />
          <Detail
            label="Kilométrage"
