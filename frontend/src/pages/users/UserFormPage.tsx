    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid gap-4 xl:grid-cols-2">
          <Card><CardHeader><CardTitle>Informations utilisateur</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><Field label="Prénom" value={form.firstname} onChange={(value) => setForm((current) => ({ ...current, firstname: capitalizeFirstLetter(value) }))} required /><Field label="Nom" value={form.lastname} onChange={(value) => setForm((current) => ({ ...current, lastname: capitalizeFirstLetter(value) }))} required /><div className="md:col-span-2"><Field label="Email" type="email" value={form.email} onChange={(value) => setForm((current) => ({ ...current, email: value }))} required /></div><div className="grid gap-2 md:col-span-2"><span className="text-sm font-medium">Rôles</span><label className="flex items-center gap-2 rounded-lg border p-3 text-sm"><input type="checkbox" checked disabled />Utilisateur</label><label className="flex items-center gap-2 rounded-lg border p-3 text-sm"><input type="checkbox" checked={form.roles.includes("ROLE_ADMIN")} onChange={() => toggleRole("ROLE_ADMIN")} />Administrateur</label></div></CardContent></Card>
...
function userToForm(user: AppUser): UserPayload {
  return { firstname: capitalizeFirstLetter(user.firstname), lastname: capitalizeFirstLetter(user.lastname), email: user.email ?? "", roles: user.roles.includes("ROLE_USER") ? user.roles : ["ROLE_USER", ...user.roles], address: { line1: user.address?.line1 ?? "", line2: user.address?.line2 ?? "", postalCode: user.address?.postalCode ?? "", city: user.address?.city ?? "", country: user.address?.country ?? "France" } }
}
